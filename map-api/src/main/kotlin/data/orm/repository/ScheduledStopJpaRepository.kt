...
interface ScheduledStopJpaRepository: JpaRepository<ScheduledStop, ScheduledStopId> {
    ...
    @Query(nativeQuery = true, value = """
        SELECT ss.stop_order, ss.arrival, ss.departure
        FROM scheduled_stop ss
        WHERE ss.journey_id = :journeyId
    """)
    fun findAllJourneyDetailsDtoByJourneyId(journeyId: Long): List<ScheduledStopJourneyDetailsDto>
}
